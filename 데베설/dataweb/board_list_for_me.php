   ...
   for ($i=$start; $i<$start+$scale && $i < $total_record; $i++)
   {
      ...
      if($row["id"] == $_SESSION['userid']){
			...
			$file_copied  = $row["file_copied"];
      		...
      		if ($row["public"] == "false")
      			$file_image = "<img src='./img/lock.png'>";
...
<?php
	if ($total_page>=2 && $page >= 2)	
	{
   	...
   	for ($i=1; $i<=$total_page; $i++)
   	{
		...
		{
			echo "<li><a href='board_list_for_me.php?page=$i'> $i </a><li>";
		}
   	}
   	if ($total_page>=2 && $page != $total_page)		
   	{
		$new_page = $page+1;	
		echo "<li> <a href='board_list_for_me.php?page=$new_page'>다음 ▶</a> </li>";
	}
	else 
		echo "<li>&nbsp;</li>";
?>
